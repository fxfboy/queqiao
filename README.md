...
## 效果 - 喷泉码
![Screenshot](screenshot-20260918-173257.png)
...
手机录制（没有 RDP、只能拿手机对着屏幕拍时）：

- 发送端启动横幅和画面状态栏会显示**建议录制时长**，按 `ceil(α × K / (fps × ρ))` 估算——α=2.0 是安全系数（peeling 解码开销约 1.2~1.5×K 个包，余量留给拍摄侧的撕裂帧/滚动快门/摩尔纹损失），ρ=0.7 是未标定时的保守单帧解出率，跑过标定后自动改用实测值。显示时 <60s 向上取整到 10s、≥60s 取整到 30s，下限 10s。
- **录多了不浪费**：视频是离线解码，解码器收齐即停，多录的尾巴不产生错误。
- **录少了可补录**：nonce 由文件内容派生，同一文件重启发送端后 nonce/K/blocklen/seed 序列完全一致，再录一段即可与前者合并解码。
