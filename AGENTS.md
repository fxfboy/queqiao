...
Backends are pluggable. Each is one module that subclasses `qr_backends.base.QRDecoderAdapter` and implements `decode_image(path) -> list[QRDecodeResult]`. Registration is explicit in `qr_backends/__init__.py`. `decoder.py` only talks to the registry (`get_backend`, `available_backends`, `DEFAULT_BACKEND`); the CLI's `--backend` choices update automatically when a new backend is registered.

To add a new backend: write `qr_backends/<name>_backend.py`, then import + register it in `qr_backends/__init__.py`.

Currently shipped:
- **zxing** (`--backend zxing`, **default**): pure-wheel C++ port of ZXing (`pip install zxing-cpp`) — no native system library required. ~10–14× faster than pyzbar on pixel-perfect QR at identical accuracy across chunk-sizes 800/1500/1800; same crash threshold under downsample+blur+JPEG degradation. See `bench_backends.py`.
- **pyzbar** (`--backend pyzbar`): wraps the native zbar library (`brew install zbar`; macOS also needs `DYLD_LIBRARY_PATH=/opt/homebrew/lib`, which `run.sh` sets). Opt-in fallback.
