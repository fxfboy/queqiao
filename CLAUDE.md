...
./run.sh decode photo1.jpg photo2.jpg -o out   # OpenCV decoder (default)
...
python decoder.py <images...> -o out [--debug]            # OpenCV path
python decode_pyzbar.py [image_dir=bid] [out=restored.out] # pyzbar path, scans a DIR
...
decoder.py:  photo → cv2 QRCodeDetector → base85 decode → parse header / verify
...
QR count = ceil(compressed_size / chunk-size). The default `--chunk-size 400` is tuned for **photographing** the screen (low density, survives lens distortion/glare). For **screenshot** transfer (pixel-perfect, lossless) you can go much larger — up to ~1800, where a single QR hits version 40 (the max; a chunk-size of ~2000+ raises a v41 error). Pair large chunks with a bigger `--qr-size` (>= the QR's native pixel size) or the screenshot's downsampling blurs the dense modules and decoding fails. Measured on a ~487KB text file: 235 codes (photo default) vs 53 codes (`chunk-size 1800`). See `output/RESULTS.md`.
...
- **OpenCV** (`decoder.py`, the `run.sh decode` default): no native lib beyond the wheel. Compensates for weaker detection by running each photo through 5 preprocessing variants (gray, adaptive threshold, Otsu, CLAHE, inverted) and both `detectAndDecodeMulti` + `detectAndDecode`.
- **pyzbar** (`decode_pyzbar.py`): generally far better at reading QRs from screen photos, but needs the **native zbar library** (`brew install zbar`; macOS also needs `DYLD_LIBRARY_PATH=/opt/homebrew/lib`, which `run.sh` sets). Takes a **directory of PNGs**, not image arguments — it's the path used against the `bid/` sample screenshots.
